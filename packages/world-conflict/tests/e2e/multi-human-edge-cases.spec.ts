...
test.describe('Multi-Human Player Advanced Edge Cases', () => {
    ...
    test('Test 12: Multiple Games Simultaneously', async ({ browser }) => {
        ...
        try {
            ...
            const gameAResult = await createGameWithSeed(player1Page, TEST_PLAYERS.PLAYER1, {
                playerSlots: [
                    {
                        type: 'Set',
            ...
            // Use API to create game directly - avoids UI blocking
            const gameBResult = await createGameWithSeed(player3Page, TEST_PLAYERS.PLAYER3, {
                playerSlots: [
                    {
                        type: 'Set',
    ...
    test('Test 13: Maximum Turn Limit', async ({ browser }) => {
        ...
        try {
            ...
            const hasGameOver = await player1Page
            ...
            if (hasGameOver) {
                ...
                const hasGameOver2 = await player2Page
                    .locator('text=/game over|ended|complete/i')
                    .isVisible({ timeout: 1000 })
                    .catch(() => false);
