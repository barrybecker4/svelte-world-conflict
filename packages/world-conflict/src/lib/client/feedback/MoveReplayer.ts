...
export class MoveReplayer {
     ...
     */
    private extractMoves(newState: GameStateData, previousState: GameStateData, regions: unknown[]): DetectedMove[] {
    ...
    private extractFromTurnMoves(
        ...
        const moves = turnMoves.map(move => this.constructMoveFromMetadata(move, newState, previousState));
     ...
     */
    private attachAttackSequenceToConquest(moves: DetectedMove[], attackSequence: unknown[], regions: unknown[]): void {
    ...
    private async playMovesSequentially(
        ...
        for (const move of moves) {
            ...
            dispatchGameEvent('battleStateUpdate', {
                gameState: currentAnimationState
            });
    ...
    constructMoveFromMetadata(
        ...
        switch (moveData.type) {
            ...
            case 'recruit':
                return {
                    type: 'recruitment',
                    regionIndex: moveData.targetRegion!
                };
            case 'upgrade':
    ...
    applyMoveToState(state: GameStateData, move: DetectedMove): GameStateData {
        ...
        if (move.type === 'conquest' && move.newOwner !== undefined) {
            newState.ownersByRegion = {
                ...newState.ownersByRegion,
                [targetRegion]: move.newOwner
            };
        }
     ...
     */
    private transferSoldiers(state: GameStateData, sourceRegion: number, targetRegion: number, count: number): void {
