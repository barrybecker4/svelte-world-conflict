...
export async function skipInstructions(page: Page) {
    ...
    try {
        ...
        await expect(instructionsModal)
 ...
 */
export async function configurePlayerSlot(page: Page, slotIndex: number, type: 'Set' | 'Open' | 'AI' | 'Off') {
 ...
 */
export async function joinExistingGame(page: Page, gameId: string, playerName: string): Promise<void> {
    ...
    try {
        ...
        const errorElement = await page
            .locator('text=/error|not found|rejoin/i')
            .first()
            .isVisible()
            .catch(() => false);
        if (errorElement) {
 ...
 */
export async function waitForPlayerToJoin(page: Page, playerName: string, slotIndex: number): Promise<void> {
...
export async function startGameAnywayFromWaitingRoom(page: Page): Promise<void> {
    ...
    try {
        await Promise.race([
            expect(waitingRoom).not.toBeVisible({
                timeout: TIMEOUTS.GAME_LOAD
            }),
            expect(gameInterface).toBeVisible({ timeout: TIMEOUTS.GAME_LOAD })
        ]);
