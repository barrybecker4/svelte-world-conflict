...
export class MoveValidator {
    ...
    static validateMove(
        ...
        if (gameData.ownersByRegion[fromRegion] !== currentPlayer.slotIndex) {
            return {
                isValid: false,
                error: "Cannot move soldiers from region you don't own"
            };
        }
        ...
        if (availableSoldiers < soldierCount) {
            return {
                isValid: false,
                error: `Not enough soldiers (have ${availableSoldiers}, need ${soldierCount})`
            };
        }
        ...
        if (availableSoldiers - soldierCount < 1) {
            return {
                isValid: false,
                error: 'Must leave at least 1 soldier in source region'
            };
        }
