...
export async function endTurn(page: Page) {
    ...
    try {
        await expect(turnIndicator).not.toContainText(currentPlayerBefore, {
            timeout: 3000
        });
...
export async function waitForTurnStart(page: Page, playerName: string, expectedTurnNumber?: number) {
    ...
    const turnIndicator = page.getByTestId('current-turn-player');
    await expect(turnIndicator).toContainText(playerName, {
...
export async function waitForTurnNumber(page: Page, expectedTurn: number) {
    ...
    await expect(turnDisplay).toHaveText(expectedTurn.toString(), {
        timeout: TIMEOUTS.TURN_TRANSITION
    });
}
...
export async function waitForAITurn(page: Page, aiPlayerName: string) {
    ...
    try {
        await expect(turnIndicator).toContainText(aiPlayerName, {
            timeout: TIMEOUTS.TURN_TRANSITION
        });

        // Wait for AI turn to complete (turn indicator changes away from AI)
        await expect(turnIndicator).not.toContainText(aiPlayerName, {
            timeout: TIMEOUTS.AI_TURN
        });
    } catch (error) {
 ...
 */
export async function synchronizeTurnTransition(pages: Page[], fromPlayer: string, toPlayer: string): Promise<void> {
    ...
    await Promise.all(pages.map(page => expect(page.getByTestId('current-turn-player')).not.toContainText(fromPlayer)));
 ...
 */
export async function verifyTurnOrder(pages: Page[], expectedPlayer: string): Promise<void> {
    ...
    results.forEach((result, idx) => {
        if (!result?.includes(expectedPlayer)) {
            throw new Error(`Page ${idx} shows wrong player: ${result}, expected: ${expectedPlayer}`);
        }
    });
...
export async function verifyTurnNumberSync(pages: Page[]): Promise<number> {
    const turnNumbers = await Promise.all(pages.map(page => getCurrentTurn(page)));
    ...
    if (!allMatch) {
        ...
        throw new Error(`Turn numbers don't match across players: ${turnNumbers.join(', ')}`);
    }
 ...
 */
export async function executeMultiPlayerTurnCycle(playersData: Array<{ page: Page; name: string }>): Promise<void> {
    ...
    for (let i = 0; i < playersData.length; i++) {
        ...
        await synchronizeTurnTransition(allPages, currentPlayer.name, nextPlayer.name);
    }
