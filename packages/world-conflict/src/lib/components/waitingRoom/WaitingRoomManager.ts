...
export class WaitingRoomManager {
    ...
    private handleSubscribed = (message: any) => {
        ...
        if (subscribedGameId !== this.gameId) {
            logger.error(
                `[WaitingRoom] Subscription game ID mismatch! Expected ${this.gameId}, got ${subscribedGameId}`
            );
    ...
    private handleDisconnected = () => {
        logger.error(
            `[WaitingRoom] WebSocket DISCONNECTED from game ${this.gameId} - this means we won't receive gameStarted notifications!`
        );
    ...
    private async setupRealtimeUpdates() {
        ...
        try {
            ...
            logger.debug(
                `Setting up WebSocket connection for game ${this.gameId} (playerId: ${playerIdStr || 'none'})`
            );
            ...
            this.wsClient.onConnected(() => {
                logger.debug(
                    `[WaitingRoom] WebSocket connected for game ${this.gameId}, waiting for subscription confirmation...`
                );
            });
