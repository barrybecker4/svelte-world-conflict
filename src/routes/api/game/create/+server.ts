...
export const POST: RequestHandler = async ({ request, platform }) => {
    try {
        const { playerName, gameType = 'MULTIPLAYER' } = await request.json() as CreateGameRequest;
        ...
        const gameData = {
            ...
            status: gameType === 'AI' ? 'ACTIVE' : 'WAITING', // AI games start immediately
            gameType,
            createdAt: Date.now(),
        ...
        console.log(`   Creator: "${playerName}" (${playerId})`);
