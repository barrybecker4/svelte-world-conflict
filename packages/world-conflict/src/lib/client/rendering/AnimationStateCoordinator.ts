...
export class AnimationStateCoordinator {
    ...
    createAttackingAnimationState(
        ...
        logger.debug(
            `Creating attacking animation state: ${soldierCount} soldiers from ${sourceRegion} to ${targetRegion}`
        );
    ...
    createPeacefulMoveAnimationState(
        ...
        logger.debug(
            `Creating peaceful move animation state: ${soldierCount} soldiers from ${sourceRegion} to ${targetRegion}`
        );
    ...
    private isConquestSuccessful(finalGameState: GameStateData, sourceRegion: number, targetRegion: number): boolean {
        const targetSoldiersInFinal = finalGameState.soldiersByRegion?.[targetRegion] || [];
    ...
    private findSurvivingAttackers(
        ...
        return currentSourceSoldiers.filter(s => s.attackedRegion === targetRegion && survivorIds.has(s.i));
    }
