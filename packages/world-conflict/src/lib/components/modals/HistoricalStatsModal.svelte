<script lang="ts">
    ...
    const gamesPlayedDatasets = $derived([
        {
            label: 'Games Started',
            data: stats.map(s => s.gamesStarted),
        ...
        {
            label: 'Multi-Human',
            data: stats.map(s => s.gamesWithMultipleHumans),
    ...
    const turnsPlayedDatasets = $derived([
        {
            label: 'Min Turns',
            data: stats.map(s => (s.minTurns === Infinity || s.minTurns === null ? 0 : s.minTurns)),
    ...
    const endReasonsDatasets = $derived([
        ...
        {
            label: 'Elimination',
            data: stats.map(s => s.endReasons?.elimination ?? 0),
            color: CHART_COLORS.red
        },
        {
            label: 'Turn Limit',
            data: stats.map(s => s.endReasons?.turnLimit ?? 0),
            color: CHART_COLORS.yellow
        },
        {
            label: 'Resignation',
            data: stats.map(s => s.endReasons?.resignation ?? 0),
            color: CHART_COLORS.purple
        }
    ]);
...
<Modal {isOpen} title="Historical Statistics" width="900px" height="85vh" onclose={handleClose}>
        ...
        {:else}
            <div class="charts-grid">
                <div class="chart-cell">
                    <LineChart title="Games Played" labels={dateLabels} datasets={gamesPlayedDatasets} height="220px" />
                </div>

                <div class="chart-cell">
                    <LineChart title="Turns Played" labels={dateLabels} datasets={turnsPlayedDatasets} height="220px" />
                </div>
