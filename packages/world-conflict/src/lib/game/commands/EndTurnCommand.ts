...
export class EndTurnCommand extends Command {
    ...
    execute(): GameState {
        ...
        logger.info(
            `EndTurnCommand: checkGameEnd called. turnNumber=${newState.turnNumber}, currentTurn=${currentTurn}, maxTurns=${newState.maxTurns}, isGameEnded=${gameEndResult.isGameEnded}, reason=${gameEndResult.reason}`
        );
        ...
        if (gameEndResult.isGameEnded) {
            ...
            logger.info(
                `🏁 Game ENDED! reason=${gameEndResult.reason}, winner=${JSON.stringify(gameEndResult.winner)}, endResult set on newState`
            );
    ...
    private getNextActiveSlot(currentSlotIndex: number, players: Player[], state: GameState): number {
        ...
        if (currentIndex === -1) {
            throw new Error(
                `Current slot ${currentSlotIndex} not found in active slots: ${JSON.stringify(activeSlots)}`
            );
        }
    ...
    private getActiveSlots(players: Player[], state: GameState): number[] {
        ...
        const activePlayers = players.filter(p => {
            const regionCount = Object.values(state.ownersByRegion).filter(owner => owner === p.slotIndex).length;
        ...
        for (const [regionIndex, templeData] of Object.entries(state.templesByRegion)) {
            ...
            if (state.ownersByRegion[regionIdx] === player.slotIndex) {
                ...
                if (airBonus > 0) {
                    ...
                    logger.debug(
                        `Player ${player.slotIndex} has Air temple at region ${regionIdx} granting ${airBonus} extra move(s)`
                    );
                }
            }
        }
