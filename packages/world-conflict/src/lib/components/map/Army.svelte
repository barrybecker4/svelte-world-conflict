<script lang="ts">
        ...
        soldiersList.forEach((soldier, index) => {
            ...
            if (movingTo !== undefined && movingTo !== regionData.index) {
                ...
                if (targetRegion) {
                    ...
                    const targetPos = calculateSoldierBasePosition(
                        finalIndex,
                        finalTotal,
                        targetRegion,
                        targetHasTemple
                    ...
                    logger.debug(
                        `Region ${regionData.index} soldier ${soldier.i} moving to ${movingTo}: positioned at ${finalIndex}/${finalTotal} at (${targetPos.x}, ${targetPos.y})`
                    );
            ...
            if (soldiersMovingToTarget.size > 0) {
                logger.debug(
                    `Region ${regionData.index} soldier ${soldier.i} staying: positioned at ${index}/${totalSoldiers} at (${basePos.x}, ${basePos.y})`
                );
            }
    ...
    function calculateSoldierBasePosition(
        index: number,
        totalSoldiers: number,
        regionData: Region,
        hasTempleAtRegion?: boolean
    ): { x: number; y: number } {
        const row = Math.floor(index / GAME_CONSTANTS.ARMIES_PER_ROW);
        const col = index % GAME_CONSTANTS.ARMIES_PER_ROW;
        const offsetX =
            (col - (Math.min(totalSoldiers, GAME_CONSTANTS.ARMIES_PER_ROW) - 1) / 2) * GAME_CONSTANTS.ARMY_OFFSET_X;

        // Use provided temple status, or default to current region's temple status
        const regionTempleOffset =
            hasTempleAtRegion !== undefined
                ? hasTempleAtRegion
                    ? GAME_CONSTANTS.TEMPLE_ARMY_OFFSET_Y
                    : 0
                : templeOffset;
    ...
    function handleBattleCasualties(event: CustomEvent) {
        ...
        if (isInvolvedInBattle) {
            logger.debug(
                `Region ${region.index}: Received battleCasualties event - source=${sourceRegion}, target=${targetRegion}, A=${attackerCasualties}, D=${defenderCasualties}`
            );
        }
        ...
        if (sourceRegion === region.index && attackerCasualties > 0) {
            ...
            const attackingSoldiers = soldierPositions.filter(
                p => !hiddenSoldierIds.has(p.soldier.i) && p.soldier.attackedRegion === targetRegion
            );
            ...
            for (let i = 0; i < attackerCasualties && i < attackingSoldiers.length; i++) {
                ...
                logger.debug(
                    `Region ${region.index}: Hiding attacker soldier ${casualty.soldier.i} at (${casualty.x}, ${casualty.y})`
                );
        ...
        if (targetRegion === region.index && defenderCasualties > 0) {
            ...
            logger.debug(
                `Region ${region.index} (DEFENDER): Hiding ${defenderCasualties} soldiers from ${visibleSoldiers.length} visible`,
                {
                    targetRegion,
            ...
            for (let i = 0; i < defenderCasualties && i < visibleSoldiers.length; i++) {
                ...
                logger.debug(
                    `Region ${region.index}: Hiding defender soldier ${casualty.soldier.i} at (${casualty.x}, ${casualty.y})`
                );
                // Hide soldier and spawn smoke simultaneously
                hiddenSoldierIds = new Set([...hiddenSoldierIds, casualty.soldier.i]);
    ...
    $: if (gameState && hiddenSoldierIds.size > 0 && !isBattleInProgress) {
        ...
        const hiddenStillExist = Array.from(hiddenSoldierIds).some(hiddenId => allSoldiers.some(s => s.i === hiddenId));
...
{#if showCountBadge}
    ...
    <circle cx={x} cy={y + templeOffset} r="10" fill="rgba(0,0,0,0.7)" stroke="#fbfbf4" stroke-width="1" />
    <text {x} y={y + templeOffset + 3} text-anchor="middle" font-size="10" font-weight="bold" fill="#fbfbf4">
        {actualSoldierCount}
    </text>
