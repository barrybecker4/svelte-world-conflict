...
export class BattleManager {
    ...
    isBattleRequired(move: BattleMove): boolean {
        ...
        const isEnemyTerritory =
            targetOwner !== undefined && targetOwner !== currentPlayerSlot && targetSoldiers.length > 0;
    ...
    async executeBattle(move: BattleMove, regions: Region[]): Promise<BattleResult> {
        ...
        try {
            const result = await this.executeBattleSequence(
                move,
                regions,
                sourceRegionIndex,
                targetRegionIndex,
    ...
    private async executeBattleSequence(
        ...
        const animationState = await this.startBattleAnimation(
            move,
            sourceRegionIndex,
            targetRegionIndex,
            soldierCount
    ...
    async executePeacefulMove(move: BattleMove): Promise<BattleResult> {
        ...
        try {
            const result = await this.executePeacefulMoveSequence(
                move,
                sourceRegionIndex,
                targetRegionIndex,
                soldierCount
