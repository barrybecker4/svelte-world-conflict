<script lang="ts">
    ...
    $: maxTurns =
        gameState?.maxTurns && gameState.maxTurns !== GAME_CONSTANTS.UNLIMITED_TURNS ? gameState.maxTurns : null;
...
>
    <Panel variant="glass" padding={false} customClass="game-info-panel">
            ...
            <div class="turn-box">
                <div class="turn-header">
                    {#if isGameOver}
                        Game over
                    {:else}
                        Turn <span class="turn-number" data-testid="turn-number">{turnNumber}</span>{#if maxTurns}
                            &nbsp;/ {maxTurns}{/if}
                    {/if}
                </div>
            </div>
            ...
            {#each players.slice().sort((a, b) => a.slotIndex - b.slotIndex) as player, slotOrderIndex}
                ...
                <div class="player-box" class:active={isActive} data-testid="player-info-{player.name}">
                    ...
                    <div class="player-info">
                        <div class="player-name" data-testid={isActive ? 'current-turn-player' : ''}>
                            {player.name}
                        </div>
            ...
            <div class="stat-display">
                <div class="stat-item">
                    <div class="stat-value">
                        {movesRemaining} <span class="symbol">{@html SYMBOLS.MOVES}</span>
                    </div>
                    <div class="stat-label">moves</div>
                </div>
                <div class="stat-item">
                    <div class="stat-value">
                        {faithByPlayer[currentPlayerSlot]}
                        <span class="symbol">{@html SYMBOLS.FAITH}</span>
                    </div>
        ...
        <Section title="" flex={true} flexDirection="column" gap="8px">
            <Button variant="secondary" uppercase disabled={!canUndo || !isMyTurn} onclick={onUndo}>↩️ Undo</Button>

            <Button
                variant="danger"
                size="lg"
                uppercase
                disabled={!isMyTurn || battleInProgress}
                onclick={onEndTurn}
