...
export class BattleAnimationUpdater {
  ...
  private setupEventListeners() {
    ...
    const movementAnimationStartListener = ((event: CustomEvent) => {
      const { targetRegion, oldOwner } = event.detail;

      // For peaceful movements, keep the region as neutral (or old owner) during the movement
      this.battleAnimationOverrides.update(overrides => ({
        soldierCounts: { ...overrides.soldierCounts },
        ownership: {
          ...overrides.ownership,
          [targetRegion]: oldOwner // Keep old ownership (undefined for neutral) during movement
        }
      }));
    }) as EventListener;
    ...
    window.addEventListener('movementAnimationStart', movementAnimationStartListener);
    ...
    this.eventListeners.push(
      ...
      { event: 'movementAnimationStart', listener: movementAnimationStartListener },
