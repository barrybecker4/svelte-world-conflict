<script lang="ts">
    ...
    $: movableRegionIndices = (() => {
        ...
        return new Set(regions.filter(region => canHighlightForTurn(region)).map(region => region.index));
    })();
...
<div class="game-map" bind:this={mapContainerElement}>
    <svg class="map-svg" viewBox="0 0 800 600" bind:this={svgElement}>
        <SvgDefinitions {gameState} {battlesInProgress} />
        ...
        <g filter="url(#regionShadow)">
            {#each regions as region (`${region.index}-${gameState?.ownersByRegion?.[region.index] ?? 'n'}`)}
                ...
                {@const isMyTurn = !!(
                    currentPlayer &&
                    gameState &&
                    currentPlayer.slotIndex === gameState.currentPlayerSlot
                )}
                {@const isClickable = !!(hasMovesRemaining && isMyTurn && !gameHasEnded)}
                {@const innerBorderOpacity = isWinner
                    ? 0.85
                    : isSelected
                      ? 0.75
                      : isValidTarget
                ...
                <RegionRenderer
                    ...
                    innerBorderColor={isSelected || isMovable || isValidTarget || isWinner
                        ? getInnerBorderColor(region, isSelected, isValidTarget, isWinner)
                        : ''}
        ...
        <g class="armies-layer">
            {#each regions as region (region.index)}
                ...
                {#if soldierCount > 0}
                    <Army x={region.x} y={region.y} {hasTemple} {gameState} {regions} {region} />
                {/if}
            {/each}
        </g>
