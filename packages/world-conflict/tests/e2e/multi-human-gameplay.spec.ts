...
test.describe('Multi-Human Player Gameplay Tests', () => {
    ...
    test('Test 10: Four Players - Territory Interactions', async ({ browser }) => {
        ...
        try {
            ...
            const gameOver1 = await player1Page
            ...
            try {
                ...
                const gameOver2 = await player1Page
                    .locator('text=/game over|ended|complete/i')
                    .isVisible({ timeout: 1000 })
                    .catch(() => false);
            ...
            const gameOver3 = await player1Page
                .locator('text=/game over|ended|complete/i')
                .isVisible({ timeout: 1000 })
                .catch(() => false);
            ...
            for (const playerData of [
                {
                    page: player2Page,
                    name: TEST_PLAYERS.PLAYER2,
                    next: TEST_PLAYERS.PLAYER3
                },
                {
                    page: player3Page,
                    name: TEST_PLAYERS.PLAYER3,
                    next: TEST_PLAYERS.PLAYER4
                },
                {
                    page: player4Page,
                    name: TEST_PLAYERS.PLAYER4,
                    next: TEST_PLAYERS.PLAYER1
                }
            ]) {
                // Check if game ended
                const gameOverCheck = await player1Page
                    .locator('text=/game over|ended|complete/i')
                    .isVisible({ timeout: 1000 })
                    .catch(() => false);
                ...
                try {
                    ...
                    const gameOverTransition = await player1Page
                        .locator('text=/game over|ended|complete/i')
                        .isVisible({ timeout: 1000 })
                        .catch(() => false);
