...
<div class="game-controls">
    ...
    <div class="action-buttons">
        <Button variant="primary" disabled={!isPlayerTurn || playerFaith < 10} onclick={onBuyArmies}
            >Buy Armies (10 Faith)</Button
        >

        <Button variant="secondary" disabled={!isPlayerTurn || playerFaith < 50} onclick={onUpgradeTemple}>
            Upgrade Temple (50 Faith)
        </Button>

        <Button variant="warning" disabled={!canUndo || !isPlayerTurn} onclick={onUndo}>↩️ Undo</Button>

        <Button variant="success" disabled={!canEndTurn || !isPlayerTurn} onclick={onEndTurn}>End Turn</Button>
    </div>
</div>
