<script lang="ts">
    ...
    $: colorScheme =
        upgradeType && TEMPLE_UPGRADES_BY_NAME[upgradeType]?.templeColors
...
>
    ...
    <ellipse cx={x + 1} cy={y + 8} rx="7" ry="3" fill="rgba(0,0,0,0.2)" />

    <!-- Main temple base (larger foundation) -->
    <ellipse cx={x} cy={y + 6} rx="6" ry="2.5" fill={darkColor} stroke={darkColor} stroke-width="0.5" />
    ...
    <polygon points="{x - 5},{y + 6} {x - 3},{y - 4} {x - 1},{y - 2} {x - 3},{y + 6}" fill={lightColor} opacity="0.6" />

    <!-- Upgrade level indicators (stacked disks) -->
    <!-- Show first disc when temple has an upgrade (level 0+) -->
    {#if upgradeType}
        <!-- First upgrade disk -->
        <ellipse cx={x} cy={y - 7} rx="2.5" ry="1" fill={discColor} stroke={darkColor} stroke-width="0.5" />
        <!-- Highlight on first disk -->
        <ellipse cx={x - 0.5} cy={y - 7.5} rx="1.5" ry="0.5" fill={lightColor} opacity="0.8" />

        <!-- Show second disc when temple is upgraded to level 1 (Cathedral) -->
        {#if upgradeLevel >= 1}
            <!-- Second upgrade disk (smaller, on top) -->
            <ellipse cx={x} cy={y - 9} rx="2" ry="0.8" fill={discColor} stroke={darkColor} stroke-width="0.5" />
            <!-- Highlight on second disk -->
            <ellipse cx={x - 0.3} cy={y - 9.3} rx="1.2" ry="0.4" fill={lightColor} opacity="0.8" />
        {/if}
    {/if}

    <!-- Temple entrance (dark doorway) -->
    <rect x={x - 1} y={y + 2} width="2" height="3" fill={darkColor} rx="1" />
</g>
