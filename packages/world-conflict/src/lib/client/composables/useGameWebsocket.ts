 ...
 */
export function useGameWebSocket(gameId: string, onGameUpdate: (gameData: GameStateData) => void, playerId?: string) {
    ...
    async function initialize(): Promise<void> {
        ...
        try {
            ...
            throw new Error(
                `WebSocket connection required but failed: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
    }
