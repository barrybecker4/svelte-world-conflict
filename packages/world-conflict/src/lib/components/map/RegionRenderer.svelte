<script lang="ts">
    ...
    $: regionPath = region.points ? regionUtil.pointsToPath(region.points) : regionUtil.createFallbackCircle(region);
    ...
    $: upgradeType =
        templeData?.upgradeIndex !== undefined ? TEMPLE_UPGRADES[templeData.upgradeIndex]?.name : undefined;
    ...
    $: effectiveFilter = isBattleInProgress ? 'url(#battleGlow)' : canHighlight ? 'url(#brightenRegion)' : undefined;
